...
#         self.can.change_mode(0)  # 0-Normal, 1-Sleep, 2-Loopback, 3-Listen Only, 4-Configuration
    ...
    def action_opcode(self, msg):
        ...
        def asc_on(msg):
            ...
            event_list = [event for event in self.data['events'] if event['event_identifier'] == event_identifier]
            if event_list > 0:
                if self.debug:
                    print("Short Event is Known : 0000" + event_identifier)
                self.execute({'task': 'off', 'variables': event_list[0]['variables']})
            else:
                if self.debug:
                    print("Event is Unknown")
        ...
        def read_ev(msg):
            if self.get_node_id(msg) == self.nodeId:
                ...
                if ev_index <= self.data["numEventVariables"]:
...
#             else:
#                 self.send(self.cmderror(2))
#                 print('EVULN : Not in Learn Mode')

        def learn_mode_on(msg):
            if self.get_node_id(msg) == self.nodeId:
                if self.debug:
                    print("NNLRN : " + msg)
                self.learn = True

        def learn_mode_off(msg):
            if self.get_node_id(msg) == self.nodeId:
                if self.debug:
                    print("NNLUN : " + msg)
                self.learn = False

        def send_all_events(msg):
            if self.get_node_id(msg) == self.nodeId:
                event_count = 1
                if self.debug:
                    print("NERD : " + msg)
